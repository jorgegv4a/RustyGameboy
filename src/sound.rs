...
impl APU {
    ...
    fn ch1_get_next_sample(&mut self, memory: &AddressSpace) -> Option<u8> {
        ...
        let value = self.ch1_sample(memory);
        Some(value)
    }

    fn ch3_get_next_sample(&mut self, memory: &AddressSpace) -> Option<u8> {
        self.ch3.period_step += 1;
        if self.ch3.period_step >= 2048 - self.ch3.period {
    ...
    pub fn ch1_tick(&mut self, memory: &mut AddressSpace) {
        ...
        if self.ch1.on {
            ...
            let outcome = self.ch1_get_next_sample(memory);
            if outcome.is_none() {
                wave_value = self.ch1_sample(memory);
                // wave_value = 255;
                // return
            } else {
                wave_value = outcome.unwrap();
            }
            // period = (2048.0 - self.ch1.period as f32)*1.0;
            period = 1.0;
        } else {
            wave_value = 255;
            period = 1.0;
        ...
        let mut sampling_ratio = TARGET_SAMPLE_RATE as f32 / (1048576.0 / period as f32);
    ...
    pub fn ch3_tick(&mut self, memory: &mut AddressSpace) {
        ...
        if self.ch3.on {
            ...
            let outcome = self.ch3_get_next_sample(memory);
            if outcome.is_none() {
                // wave_value = 0;
                return
            } else {
            ...
            period = (2048 - self.ch3.period) as f32;
        } else {
            wave_value = 255;
            period = 1.0;
        }
